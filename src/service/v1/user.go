...
import (
	...
	"github.com/xavier-web3/XavSwapBackend/src/types/v1"
...
func GetUserLoginMsg(ctx context.Context, svcCtx *svc.ServiceCtx, address string) (*types.UserLoginMsgResp, error) {
	uuid := uuid.NewString()
	loginMsg := genLoginTemplate(uuid)
	if err := svcCtx.KvStore.Setex(getUserLoginMsgCacheKey(address), uuid, 72*60*60); err != nil {
		return nil, err
	}
	return &types.UserLoginMsgResp{Address: address,
		Message: loginMsg}, nil
}
